...
public class Portal : MonoBehaviour {
    ...
    public void Render () {
        ...
        if (!alwaysRender && !CameraUtility.VisibleFromCamera (linkedPortal.screen, playerCam)) {
            return;
        }
    ...
    float ProtectScreenFromClipping (Vector3 viewPoint) {
        ...
        screenT.localPosition = Vector3.forward * screenThickness * ((camFacingSameDirAsPortal) ? 0.5f : -0.5f)/100;
    ...
    void OnTravellerEnterPortal (PortalTraveller traveller) {
        if (!trackedTravellers.Contains (traveller)) {
            ...
            Debug.Log("Portal: " + transform.name);
